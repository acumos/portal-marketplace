				...
				'manageModule',
				{
					...
					controller : function($scope, $compile, $location, $http, $q,
						...
						$scope.getPrivateModels=function(){
							...
							var previousContent = $scope.mlSolutionPrivate;
							...
							$http({
								...
								if($scope.seeAllSelected && $scope.hidePrivate){
/*									previousContent = previousContent.concat(data.response_body.content);
									$scope.mlSolutionPrivate.length = 0;
									
									angular.forEach(previousContent, function(value,key) {
										$scope.mlSolutionPrivate.push({'name':'test'});
									});*/
									
									/*count = (data.response_body.content).length;
									angular.forEach(data.response_body.content, function(value,key) {
										$scope.mlSolutionPrivate.push(value);
									});*/
/*									for(var count = 0; count < (data.response_body.content).length; count++ ){
										$scope.mlSolutionPrivate[(mlSolutionPrivate.length)+count]=data.response_body.content[count];
									}*/
									//$scope.mlSolutionPrivate.concat([(data.response_body.content[1])]);
									//$scope.$apply();
								} else {
						...
						$scope.getCompanyModels=function(){
							...
							var url = '/api/models/' + $scope.loginUserID;
							toBeSearch = $scope.searchBox;
							var mlSolutionCompany = $scope.mlSolutionCompany;
							dataObj = {
									"request_body" : {
										"modelType" : $scope.categoryFilter,
										"accessType" : 'OR',
							...
							$http({
								...
								if($scope.seeAllSelected && $scope.hideCompany){
									$scope.mlSolutionCompany = (mlSolutionCompany).concat($scope.mlSolutionCompany);
								}
						...
						$scope.getPublicModels=function(){
							...
							var url = '/api/models/' + $scope.loginUserID;
							toBeSearch = $scope.searchBox;
							var mlSolutionPublic = $scope.mlSolutionPublic;
							...
							$http({
								...
								if($scope.seeAllSelected && $scope.hidePublic){
									$scope.mlSolutionCompany = (mlSolutionPublic).concat($scope.mlSolutionPublic);
								}
						...
						$scope.getDeleteModels=function(){
							...
							/*if ($scope.isBusy)
								...
								$scope.isBusy = true;*/
							var url = '/api/models/' + $scope.loginUserID;
							var mlSolutionDelete = $scope.mlSolutionDelete;
							toBeSearch = $scope.searchBox;
							dataObj = {
									"request_body" : {
										"modelType" : $scope.categoryFilter,
							...
							$http({
								...
								if($scope.seeAllSelected && $scope.hideDelete){
									$scope.mlSolutionDelete = (mlSolutionDelete).concat($scope.mlSolutionDelete);
								}
                        ...
                        $scope.publicNav=function(NavName){
                        ...
                        $scope.CompanyNav=function(NavName){
						...
						$scope.privateNav=function(NavName){
                        ...
                        $scope.deleteNav=function(NavName){
