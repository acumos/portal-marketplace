...
function ACController($scope,$http,$filter,$q,$window,$rootScope,$mdDialog ,$state,$stateParams,$injector, browserStorageService, apiService) {
	componentHandler.upgradeAllRegistered();
	 ...
	 function getCookieValue(userInfo) {
	    var b = document.cookie.match('(^|[^;]+)\\s*' + userInfo + '\\s*=\\s*([^;]+)');
	    return b ? b.pop() : '';
	}

	 //$scope.userDetails = JSON.parse(browserStorageService.getUserDetail());
	 $scope.userDetails = getCookieValue(userInfo);
    ...
    function get_userId() {
        var userDetail = getCookieValue(userInfo);
        //userDetail = !userDetail ? getCookieValue(userDetail);
        if(!userDetail){
        	var modalService = $injector.get('$mdDialog'); 
		modalService.show({
			 templateUrl: '../app/header/sign-in-promt-modal-box.html',
