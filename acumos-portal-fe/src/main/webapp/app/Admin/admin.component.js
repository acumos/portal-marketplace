...
angular.module('admin').filter('abs', function () {
    ...
    .component('admin', {
        ...
        controller: function ($scope, apiService, fileUploadService, $mdDialog, $http, $timeout, $location, $anchorScroll, $uibModal, $rootScope, $state, $filter, browserStorageService) {
            ...
            $scope.showPopupPeeR1 = function (ev, val) {
                ...
                $http.post(url).success(function (response) {
                    ...
                    angular.forEach(response.response_body, function (value, key) {
                    	$scope.getCatalogName(value.subId);
            ...
            //get Catalog Names
            $scope.getCatalogName = function (valSubid) {
            	$scope.catalogNameArr = [];
            	$scope.arrDetailsList = [];
            	
            	apiService.getSubsDetails(valSubid)
             	 .then(
         			 function(response) {
         				
         				 $scope.catalogNameArr.push({
         					"catalogName": response.data.response_body.catalogName,
         				 	"subId" : response.data.response_body.subId
         				 });
         				         				 
         				angular.forEach($scope.arrSub, function (value1, key1) {
         					angular.forEach($scope.catalogNameArr, function (value2, key2) {
         						
         						if (value1.subId == value2.subId) {
         							$scope.arrDetailsList.push({
         	                            "subId": value1.subId,
         	                            "updatedOn": value1.updatedOn,
         	                            "createdOn": value1.createdOn,
         	                            "frequencySelected": value1.frequencySelected,
         	                            "catalogName": value2.catalogName
         	                        })
         				...
         				$scope.count =0;
         				$scope.found = false;
         				 for( var i=0; i < $scope.arrDetailsList.length; i++){
         		          for(var j=0;j<$scope.uniqueArray.length;j++){
         		        	  
         		        	  if($scope.arrDetailsList[i].subId == $scope.uniqueArray[j].subId){
         		        		 $scope.found = true;
         		        	  }
         		        	  }
         		          
         		         $scope.count++;
         		          if($scope.count == 1 && $scope.found == false){
         		        	 $scope.uniqueArray.push($scope.arrDetailsList[i]);
         		          }
         		         $scope.count = 0;
         		        $scope.found =false;
         		          }
         		         
         				$scope.arrDetails = $scope.uniqueArray;
         				$rootScope.setLoader = false;
          ...
          //update frequency based on peer id subscription  ends
            ...
            $scope.backTo = function () {
              ...
              $scope.addedAllToSubs = false;
                ...
                $http.post(url).success(function (response) {
                    ...
                    $scope.arrSub = [];
                    angular.forEach(response.response_body, function (value, key) {
                    	$scope.getCatalogName1(value.subId);
                        var catTool = value.selector;
                        var catTool = catTool.split(",");
                        if (catTool.length > 1) {
                            angular.forEach($scope.category, function (value, key) {
            ...
            $scope.getCatalogName1 = function (valSubid) {
            	$scope.catalogNameArr1 = [];
            	$scope.arrDetailsList1 = [];
            	
            	apiService.getSubsDetails(valSubid)
             	 .then(
         			 function(response) {
         				
         				 $scope.catalogNameArr1.push({
         					"catalogName": response.data.response_body.catalogName,
         				 	"subId" : response.data.response_body.subId
         				 });
         				 
         				angular.forEach($scope.arrSub, function (value1, key1) {
         					angular.forEach($scope.catalogNameArr1, function (value2, key2) {
         						if (value1.subId == value2.subId) {
         							$scope.arrDetailsList1.push({
         	                            "subId": value1.subId,
         	                            "updatedOn": value1.updatedOn,
         	                            "createdOn": value1.createdOn,
         	                            "frequencySelected": value1.frequencySelected,
         				...
         				$scope.count1 =0;
         				$scope.found1 = false;
         				 for( var i=0; i < $scope.arrDetailsList1.length; i++){
         		          for(var j=0;j<$scope.uniqueArray1.length;j++){
         		        	  
         		        	  if($scope.arrDetailsList1[i].subId == $scope.uniqueArray1[j].subId){
         		        		 $scope.found1 = true;
         		        	  }
         		        	  }
         		          
         		         $scope.count1++;
         		          if($scope.count1 == 1 && $scope.found1 == false){
         		        	 $scope.uniqueArray1.push($scope.arrDetailsList1[i]);
         		          }
         		         $scope.count1 = 0;
         		        $scope.found1 =false;
         		          }
         		         
         				$scope.arrDetails = $scope.uniqueArray1;
         				$rootScope.setLoader = false;	
