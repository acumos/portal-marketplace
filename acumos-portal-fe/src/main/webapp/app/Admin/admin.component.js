		...
		.component(
				'admin',
				{
					...
					controller : function($scope, apiService,
							fileUploadService, $mdDialog, $http, $timeout,
							$location, $anchorScroll, $uibModal, $rootScope,
							$state, $filter, browserStorageService) {
						componentHandler.upgradeAllRegistered();
						// Editor Module configuration
						$scope.modulesConfig = {
							toolbar : [ [ 'bold', 'italic', 'underline' ], // toggled
																			// buttons
							[ 'link' ] ]
						};
						// Sorting
						...
						$scope.orderByFieldFed = 'created';
						$scope.reverseSortFederation = true;
						...
						$scope.verifyUrl = function(apiUrlPop) {
							...
							console.log(url);
							/*
							 * var json={"request_body": { "apiUrl": apiUrlPop, }}
							 */
							$http({
								method : "GET",
								url : url
									...
									.then(
											function mySuccess(response) {
												...
												if (response.data.status_code == 400) {
													...
													$scope.errorMessage = response.data.error_code
															+ ": "
						...
						$scope.browseForCatTool = function() {
							...
							if ($scope.toolKitTypeValue) {
								toolKit = '"toolkitTypeCode":"'
										+ $scope.toolKitTypeValue
							}
							if (cat && toolKit) {
								var catToolkit = '{' + cat + ',' + toolKit
										+ '"}';
							} else if (cat && !toolKit)
								var catToolkit = '{' + cat + '}';
							else if (!cat && toolKit)
								var catToolkit = '{' + toolKit + '"}';

							console.clear();
							console.log(catToolkit);
							var json = {
								"request_body" : {
							...
							console.log(json);
							console.log($scope.publicSolList);
							$http({
								method : "POST",
						...
						/*
						 * $scope.subscription = [{ "requestId": "REQID
						 * 12345680", "requestedDetails": "Requested Details",
						 * "requestType": "Model Download", "sender": "Techm10",
						 * "date": 1518252201559, "action": null, "status":
						 * 'pending' }]
						 */
						$scope.activeRequest = 5;
						// Hard coded (delete it)
						$scope.freqOfUpdate = 'hr';
						$scope.menuName = 'Monitoring';
						...
						$scope.checkAdmin = function() {
							if (browserStorageService.getUserRole() == 'Admin'
									|| browserStorageService.getUserRole() == 'admin') {
						...
						// API for get Roles
						function getRole() {
							apiService.getRoleCount() // .getAllRole()
														// //.getRoleCount()
						...
						// API for get Roles
						function getAllRole() {
							apiService.getAllRole() // .getAllRole()
													// //.getRoleCount()
							.then(function(response) {
							...
							apiService
									...
									.then(
											function(response) {
												...
												$scope.alluserDetails = ($scope.alluserDetails)
														.filter(function(
																userObj) {
						...
						$scope.showOrderEventSlidesPopup = function(ev,
								changeKey) {
							$scope.changeOrderfor = changeKey;
							$mdDialog.show({
						...
						$scope.confirmDeleteTopCarousel = function(ev,
								deleteKey) {
						...
						$scope.confirmDeleteEventCarousel = function(ev,
								deleteKey) {
							$scope.deleteKey = deleteKey;
							$mdDialog.show({
						...
						$scope.confirmDeleteStoryCarousel = function(ev,
								deleteKey) {
							$scope.deleteKey = deleteKey;
							$mdDialog.show({
						...
						$scope.showOrderStorySlidesPopup = function(ev,
								changeKey) {
							$scope.changeOrderfor = changeKey;
							$mdDialog.show({
						...
						$scope.submitFucn = function() {
							if ($scope.signupForm.$invalid || !$scope.roleValue) {
								return;
							}
									...
									.then(
											function(response) {
												if (response.data.error_code == 500) {
													...
													$scope.data = null;
													$scope.roleValue = '';
													$scope.signupForm.fname.$touched = false;
													$scope.signupForm.lname.$touched = false;
													$scope.signupForm.uname.$touched = false;
													$scope.signupForm.email.$touched = false;
													$scope.signupForm.pwd.$touched = false;
													$scope.signupForm.cpwd.$touched = false;
													$scope.signupForm.pwd.$dirty = false;

													$mdDialog.hide();
													$location.hash('myDialog'); // id
																				// of a
																				// container
																				// on
																				// the
						...
						apiService
								...
								.then(
										function(response) {
											$scope.siteConfig = angular
													.fromJson(response.data.response_body.configValue);
										}, function(error) {
											console.log(error);
										});
						...
						apiService
								...
								.then(
										function(response) {
											...
											$scope.termcondition2 = angular
													.copy($scope.termcondition1);
										}, function(error) {
											console.log(error);
										});
						...
						apiService
								...
								.then(
										function(response) {
											...
											$scope.contactInfo1 = angular
													.copy($scope.contactInfo);
										}, function(error) {
											console.log(error);
										});
						...
						$scope.TopostContactupdate = function() {
							...
							for (var i = 0; i < $scope.siteContenttInfo.length; i++) {
								arrayCheck.push($scope.siteContenttInfo
										.charCodeAt(i));
							}
							...
							apiService.updateContactInfo(toSendrequest).then(
									function(response) {
										$location.hash('myDialog'); // id of a
																	// container
																	// on the
																	// top of
																	// the page
						...
						$scope.TopostConditionupdate = function() {
							...
							apiService.updateTermsConditions(
									toSendConditionrequest).then(
									function(response) {
										$location.hash('myDialog'); // id of a
																	// container
																	// on the
						...
						$scope.submitForm = function() {
									...
									.forEach(
											...
											function(value, key) {
												if ($scope.siteConfig.fields[key].type == 'file') {
													 ...
													 * var fileObject = {
													 * 'lastModified' :
													 * $scope.siteConfig.fields[key].data.lastModified,
													 * 'lastModifiedDate' :
													 * $scope.siteConfig.fields[key].data.lastModifiedDate,
													 * 'name' :
													 * $scope.siteConfig.fields[key].data.name,
													 * 'size' :
													 * $scope.siteConfig.fields[key].data.size,
													 * 'type' :
													 * $scope.siteConfig.fields[key].data.type };
													 */
							...
							apiService
									...
									.then(
											function(response) {
												// $scope.siteConfig =
												// angular.fromJson(response.data.response_body.configValue);
												angular
														.forEach(
																$scope.siteConfig.fields,
																function(value,
																	...
																	if ($scope.siteConfig.fields[key].name == 'coBrandingLogo'
																	...
																	if ($scope.siteConfig.fields[key].name == 'Choose Background color'
																			&& $scope.siteConfig.fields[key].data
																			&& $rootScope.coBrandingImage) {
																		$rootScope.coBrandingBg = $scope.siteConfig.fields[key].data;
																	}
																	if ($scope.siteConfig.fields[key].name == 'Add tooltip to logo'
																			&& $scope.siteConfig.fields[key].data
																			&& $rootScope.coBrandingImage) {
						...
						function fetchCat() {
							...
							apiService.getModelTypes().then(function(response) {
						...
						$scope.addPeer = function() {
							if ($scope.itsEdit == true) {
								$scope.updatePeer('detail');
								return
							}
							var peerDetails = {
								"request_body" : {
									...
									"statusCode" : "IN"
								// "selector":
								// $scope.queryParam/*"{\"CL\":\"Classification\",\"DT\":\"Data
								// Transform\"}"*/
								}
							};
							apiService
									.insertPeers(peerDetails)
									.then(
											function(response) {
						...
						$scope.editPeer = function(peerDetail) {
							...
							$scope.peerNamePop = peerDetail.name;
							$scope.subNamePop = peerDetail.subjectName;
							$scope.emailIdPop = peerDetail.contact1;
							$scope.apiUrlPop = peerDetail.apiUrl;
							$scope.webUrlPop = peerDetail.apiUrl;
							$scope.descriptionPop = peerDetail.description;
							$scope.prevSubNamePop = peerDetail.subjectName;
							$scope.prevApiUrlPop = peerDetail.apiUrl;

							$scope.showPopupPeer();
						}
						$scope.checkUrlChange = function() {
							if ($scope.prevSubNamePop != $scope.subNamePop
									|| $scope.prevApiUrlPop != $scope.apiUrlPop) {
								$scope.verified = null;
								$scope.errorMessage = "";
								$scope.verify = true;
							}
						...
						$scope.isSelfTrueFun = function(peerDetail, isSelf) {
							/*
							 * if ( isSelf == true){ $scope.isSelf = true;
							 * }else{ $scope.isSelf = false; }
							 */
						...
						$scope.updatePeer = function(val) {
							...
							apiService
									.editPeer($scope.editPeerID, peerDetails)
									.then(
											function(response) {
												$scope.isSelfTrue = false;
												$scope.peer = '';
						...
						$scope.catChange = function(val1, val2) {
							...
							angular.forEach($scope.toolKitType, function(value,
									key) {
						...
						$scope.modelEdit = function() {
							...
							apiService
									.getSolutionPicture($scope.modelIDValue)
									.then(
											function(response) {
												if (response.data.response_body)
													$scope.imgURLdefault = "data:image/jpeg;base64,"
							...
							var url = "api/gateway/" + $scope.modelIDValue
									+ "/solution/" + $scope.peerIdForSubsList;
							$http({
								method : "GET",
								url : url
							})
						...
						$scope.showPopupPeeR1 = function(ev, val) {
							...
							var url = 'api/admin/peer/subcriptions/'
									+ val.peerId;
							$http
									.post(url)
									.success(
												...
												angular
														.forEach(
																...
																function(value,
																		...
																		angular
																				.forEach(
																						...
																						function(
																							...
																							var serchValue = catTool[0]
																									.search(serch);
																							if (serchValue > 0)
																								$scope.categoryForSubId = value;
																						});
																		angular
																				.forEach(
																						$scope.toolKitType,
																						function(
																								...
																								key) {
																							var serch = value.code;
																							var serchValue = catTool[1]
																									.search(serch);
																							if (serchValue > 0)
																								$scope.toolKitForSubId = value;
																						});
																	} else {
																		$scope.toolKitForSubId = '';
																		$scope.categoryForSubId = '';
																		if (catTool[0]
																				.search('modelTypeCode') > 0) {
																			angular
																					.forEach(
																							$scope.category,
																							function(
																	...
																	/*
																	 * angular.forEach($scope.frequency,
																	 * function(value1,
																	 * key1) {
																	 */
						...
						$scope.addToSubs = function() {
							...
							if (!$scope.categoryValue
									&& !$scope.toolKitTypeValue) {
								// changed since code was getting fetched in
								// modelType earlier
								/*
								 * $scope.categoryValue =
								 * $scope.solutionDetail.modelType;
								 * $scope.toolKitTypeValue =
								 * $scope.solutionDetail.tookitType;
								 */
						...
						$scope.deleteSub = function(subId, index) {
							// deleteSubscription
							apiService.deleteSubscription(subId).then(
									function(response) {
						...
						// Delete Peer
						/*
						 * $scope.deletePeer = function(peerId, idx) { idx= '';
						 * $scope.idx = idx; $scope.peerId = peerId; var
						 * modalInstance = $uibModal.open({ templateUrl:
						 * 'deletePeerModal.html', controller:
						 * 'ModalInstanceCtrl', windowClass: 'modal fade
						 * peerpopup in', resolve: { params: function() { return
						 * {param: $scope.peerId, idx:$scope.idx}; } } }); };
						 */
						...
						$scope.deletePeerFunc = function() {
							...
							apiService
									.deactivatePeer($scope.selectedPeer.peerId,
											peerDetails)
									.then(
											function(response) {
												getAllPeer();
							...
							/*
							 * apiService .deletePeer($scope.selectedPeer)
							 * .then( function(response){
							 * 
							 * getAllPeer(); fetchPeer(); $scope.closePoup();
							 * $location.hash('myDialog'); // id of a container
							 * on the top of the page - where to scroll (top)
							 * $anchorScroll(); $scope.msg = "Peer deleted
							 * successfully."; $scope.icon = '';
							 * $scope.styleclass = 'c-success';
							 * $scope.showAlertMessage = true;
							 * $timeout(function() { $scope.showAlertMessage =
							 * false; }, 5000); // success },
						...
						function perValue() {
							$scope.permissionList = [ {
								permission : 'Design Studio',
								value : 'DS'
							}, {
								permission : 'Market Place',
								value : 'MP'
							}, {
						...
						$scope.permissionValue = function(val) {
							...
							$scope.perString = '';
							$scope.perList = '';
							angular.forEach(permissionList,
									function(value, key) {
							...
							if (checkDup) {
								permissionList.splice(dupKey, 1);
							} else
								permissionList.push({
									value : val.value
								});
							angular.forEach(permissionList,
									function(value, key) {
										$scope.perString = $scope.perString
												+ '"' + value.value + '",';
									});
						...
						$scope.roleCheckbox = function(selectBox, obj) {
							...
							if (selectBox == true) {
								...
								$scope.roleArr = jQuery.grep($scope.roleArr,
										function(value) {
						 ...
						 * if(value.userId == obj.userId){ keyVal= key;dupInMap =
						 * true; } });
						...
						$scope.addRoleFuncConfirm = function(ev) {
							if ($scope.functionCall == 'updateValue') {
								...
								$scope.confirmMsg = "Do you want to "
										+ $scope.activeYN + " users ?";
								$scope.warningMsg = $scope.activeYN[0]
										.toUpperCase()
										+ $scope.activeYN.slice(1)
										+ " Confirmation";
							}
							$mdDialog.show({
						...
						$scope.addRoleFunc = function(val) {
							if (val == 'update') {
								var json = [];
								/*
								 * angular.forEach($scope.roleArr,
								 * function(value, key) { json.push({ "roleId":
						...
						$scope.deleteUser = function() {
							...
							apiService.deleteUser(obj).then(function(response) {
								$scope.activeYN = '';
								userDetailsFetch();
								$scope.roleArr = [];
								$scope.closePoup();
						...
						$scope.userFilter = function(role) {
							if (role == 'all') {
								$scope.userDetails = $scope.alluserDetails;
								return;
							}
							var temp = [];
						...
						$scope
								.$watch(
										...
										function(val) {
											if (val
													&& val.length == 0
													&& ($scope.searchUserDetails || $scope.searchPeerDetails)
													&& ($scope.searchUserDetails.firstName || $scope.searchPeerDetails.name)) {
						...
						/*
						 * Have Added this function for the count as for now.
						 * This is not good for the performance. Please remove
						 * and ask BE team for an api to return the counts
						 * 
						 * Have removed the subscription count -
						 */
						/*
						 * $scope.countSubscriptions = function(){
						 * $scope.subscriptionCount = []; $scope.peer;
						 * if($scope.peer){ angular.forEach($scope.peer,
						 * function(value, key) { value.peerId; var counyUrl =
						 * 'api/admin/peer/subcriptions/' + value.peerId;
						 * $http.post(counyUrl).success(function(response){
						 * //$scope.subscriptionCount =
						 * response.response_body.length;
						 * $scope.subscriptionCount.push( { "peerId" :
						 * value.peerId, "subscriptionLength" :
						 * response.response_body.length, } )
						 * 
							...
							$http
									...
									.error(
											function(error) {
												...
												console
														.error("Error fetching subscription counts");
											});
						}
						...
						$scope.addAllSolutions = function() {
							...
							// angular.forEach($scope.publicSolList,function(value,
							// key) { //mlsolutionCatTool
							// angular.forEach($scope.mlsolutionCatTool,function(value,
							// key) {
							...
							catToolkit = "";
							/*
							 * if(value.modelTypeCode){ cat =
							 * '"modelTypeCode":"' +value.modelTypeCode + '"' }
							 * if(value.toolkitTypeCode){ toolKit =
							 * '"toolkitTypeCode":"' +value.toolkitTypeCode }
							 * if(cat&&toolKit) catToolkit = '{' + cat + ',' +
							 * toolKit + '"}';
							 */
							if ($scope.selectedCatalogId !== "undefined"
							...
							// });
							var reqAddObj = {
								"request_body" : addAllSolObj
							}
							console.clear();
							console.log(reqAddObj);
							apiService
									.insertAddAllSolutions(
											$scope.peerIdForSubsList, reqAddObj)
									.then(
											function(response) {
												console.log(response);
												fetchToolKitType();
						...
						/*
						 * fetch number of subscriptions per peer. Currently
						 * invoking api from fe Need counts from BE since
						 * performance reduces.
						 */
						...
						var onBoardingStep = {
							"step" : [ {
								...
								"stepName" : "Create TOSCA",
								"class" : "create-tosca"
							}, {
								"stepName" : "Security Scan",
								"class" : "create-security"
							}, {
								"stepName" : "Add to Repository",
						...
						var localFlowStep = {
							"step" : [ {
								"stepName" : "Model Documentation",
								"class" : "create-model-documentation"
							},
						...
						$scope.optionalLocalFlowStep = {
							"step" : [ {
								"stepName" : "Security Scan",
								"class" : "create-security-gray",
								"active" : "true"
							}, {
								"stepName" : "License Check",
								"class" : "create-licence-check",
								"active" : "true"
							}, {
								"stepName" : "Text Check",
								"class" : "create-text-check",
								"active" : "true"
							} ]
						};
						var publicFlowStep = {
						...
						$scope.optionalPublicFlowStep = {
							...
							// { "stepName" : "Manual Text Check", "class" :
							// "create-manual-text-check", "active" : "true"}
							]
						};
						var fedratedStep = {
							"step" : [ {
						...
						$scope.showPrerenderedDialog = function(ev, dialogId,
								workFlow) {
							$scope.workFlow = workFlow;
							if (workFlow == "On-boarding Work flow") {
								$scope.workFlowStep = onBoardingStep;
							} else if (workFlow == "Publishing to Local Work Flow") {
								$scope.workFlowStep = localFlowStep;
								$scope.optionalWorkFlowStep = $scope.optionalLocalFlowStep;
							} else if (workFlow == "Publishing to Public Work Flow") {
								$scope.workFlowStep = publicFlowStep;
								$scope.optionalWorkFlowStep = $scope.optionalPublicFlowStep;
							} else if (workFlow == "Import Federated Model Work Flow") {
								$scope.workFlowStep = fedratedStep;
							}
							$mdDialog.show({
									...
									.then(
											function(response) {
												...
												if (flowConfigKey == "local_validation_workflow") {
													...
													$scope
															.getValidationWorkflow("public_validation_workflow");
												} else if (flowConfigKey == "public_validation_workflow") {
						...
						$scope
								.getValidationWorkflow("local_validation_workflow");
						...
						$scope.addValidationStep = function(validStep, validKey) {
							...
							if ($scope.optionalWorkFlowStep.step[validKey].active == "true") {
								...
								var index = $scope.ignoreWorkFlow.ignore_list
										.indexOf(validStep);
								$scope.ignoreWorkFlow.ignore_list.splice(index,
										1);
								$scope.optionalWorkFlowStep.step[validKey].active = "true";
								$scope.added = "true";
							}
							angular.forEach($scope.optionalWorkFlowStep.step,
							...
							if ($scope.workFlow == "On-boarding Work flow") {
								$scope.assignOnBoardingActivate = true;
							} else if ($scope.workFlow == "Publishing to Local Work Flow") {
								$scope.assignLocalActivate = true;
								$scope.activeLocalFlowStep = $scope.activeworkFlowStep;
							} else if ($scope.workFlow == "Publishing to Public Work Flow") {
								$scope.assignPublicActivate = true;
								$scope.activePublicFlowStep = $scope.activeworkFlowStep;
							} else if ($scope.workFlow == "Import Federated Model Work Flow") {
								$scope.assignFedratedActivate = true;
							}
							$timeout(function() {
								$scope.added = "";
							}, 5000);
						...
						$scope.assignWorkFlow = function(flow) {
							var configKey = "";
							if (flow == "Publishing to Local Work Flow") {
								configKey = "local_validation_workflow";
							} else if (flow == "Publishing to Public Work Flow") {
								configKey = "public_validation_workflow";
							}
							$scope.assignLocalActivate = false;
							$scope.assignPublicActivate = false;
							var data = angular.copy($scope.ignoreWorkFlow);
							var strworkFlowConfig = JSON.stringify(data);
							var convertedString = strworkFlowConfig.replace(
									/"/g, '\"');
							var reqObj = {
								"request_body" : {
									"configKey" : configKey,
							...
							apiService.updateSiteConfig(configKey, reqObj)
									.then(function(response) {
						...
						$scope.uploadLogoImg = function() {
							if ($scope.logoImage) {
								...
								var ext = fileName.split('.').pop()
										.toLowerCase();// substr($('#userImage').value.lastIndexOf('.')+1);
								var size = file.size;
						...
						$scope.searchData = function(searchValue) {
							var rolename = "";
							angular.forEach(searchValue.userAssignedRolesList,
									function(userAssignedRolesList) {
										rolename = userAssignedRolesList.name;
									});

							return (angular.lowercase(searchValue.username)
									.indexOf(
											angular.lowercase($scope.search)
													|| '') !== -1
									|| angular
											.lowercase(searchValue.emailId)
											.indexOf(
													angular
															.lowercase($scope.search)
															|| '') !== -1
									|| angular
											.lowercase(searchValue.status)
											.indexOf(
													angular
															.lowercase($scope.search)
															|| '') !== -1 || angular
									.lowercase(rolename).indexOf(
											angular.lowercase($scope.search)
													|| '') !== -1);
						};
						...
						$scope.bgColor = [ '#0366d6', '#10A6B5', '#8529f5',
								'#7B132A', '#D5305A', '#F49419' ];
						...
						$scope.isSlideValid = function(slide, type) {
							...
							valid = valid && slide.name
									&& slide.name.length > 0;
							if ([ "top", "event" ].includes(type)) {
								valid = valid && slide.headline
										&& slide.headline.length > 0;
								valid = valid
										&& $scope.isWithinCharLimit(
												slide.headline,
												$scope.headlineCharLimit);
								valid = valid
										&& (!slide.graphicImgEnabled
												|| $scope.carouselSlide.infoGraphic || $scope.carouselSlide.hasInfoGraphic);
								valid = valid && slide.textAling;
								valid = valid
										&& (!slide.supportingContent || $scope
												.isWithinCharLimit(
														(type == "top") ? $scope.topSC
																: $scope.eventSC,
														$scope.scCharLimit));
							} else if (type == "story") {
								valid = valid && slide.authorName
										&& slide.authorName.length > 0;
								valid = valid
										&& $scope.isWithinCharLimit(
												slide.authorName,
												$scope.headlineCharLimit);
						...
						$scope.addCarouselSlide = function() {
							...
							if ((!$scope.carouselSlide.backGround || !$scope
									.validateImageSize(
											$scope.carouselSlide.backGround,
											524, 2560))
									&& (!$scope.carouselSlide.infoGraphic || !$scope
											.validateImageSize(
								...
								if (typeof $scope.carouselConfig === "undefined") {
									...
									var keys = Object
											.keys($scope.carouselConfig);
									var carousel = $scope.carouselConfig;
									var uniqueKeys = Object.values(
											$scope.carouselConfig).map(
											function(item) {
								...
								if (keys.length == undefined
									...
									var uniqueIndex = uniqueKeys.sort(function(
											a, b) {
										return b - a
									})[0];
								}
								...
								slide['bgImgKey'] = "carousel.top."
										+ slide['uniqueKey'] + ".bgImg";
								slide['infoImgKey'] = "carousel.top."
										+ slide['uniqueKey'] + ".infoImg";
								slide['hasInfoGraphic'] = $scope.carouselSlide.hasInfoGraphic
										|| ($scope.carouselSlide.infoGraphic != null);

								if ($scope.carouselSlide.links) {
									links['enableLink'] = $scope.carouselSlide.links.enableLink;
									links['primary'] = {};
									if (!angular
											.isUndefined($scope.carouselSlide.links.primary)) {
									...
									links['secondary'] = {};
									if (!angular
											.isUndefined($scope.carouselSlide.links.secondary)) {
								...
								var carouselConfigStr = JSON
										.stringify(carousel);

								var convertedString = carouselConfigStr
										.replace(/"/g, '\"');
								...
								// if carouselConfig is not present in DB create
								// a record else update the record
						...
						$scope.deleteCarouselSlide = function() {
							...
							$scope
									.deleteCarouselPicture($scope.carouselConfig[key]['bgImgKey']);
							$scope
									.deleteCarouselPicture($scope.carouselConfig[key]['infoImgKey']);
							var updatedCarouselConfig = [];
							for (var i = 0; i < Object
									.keys($scope.carouselConfig).length; i++) {
							...
							var carouselConfigStr = JSON
									.stringify($scope.carouselConfig);
							var convertedString = carouselConfigStr.replace(
									/"/g, '\"');
							...
							apiService.updateSiteConfig("carousel_config",
						...
						$scope.editCarouselSlide = function(key, val) {
							...
							if ($scope.topSC == "") {
								...
								$scope.topSC = val['supportingContent']
										.replace(/<(?:.|\n)*?>/gm, '');
								$scope.keyval = key;
								$scope.showAddSlidesPopup();
							}
						...
						$scope.changeCarouselSlideOrder = function(ev) {
							...
							$scope.changeOrderfor = parseInt(
									$scope.changeOrderfor, 10);
							$scope.order.changeOrderValue = parseInt(
									$scope.order.changeOrderValue, 10);
							if (isNaN($scope.order.changeOrderValue)
									|| $scope.order.changeOrderValue > Object
											.keys($scope.carouselConfig).length - 1
									|| $scope.order.changeOrderValue < 0) {
							...
							var tempvalue = updatedCarouselConfig.splice(
									$scope.changeOrderfor, 1);
							updatedCarouselConfig.splice(
									$scope.order.changeOrderValue, 0,
									tempvalue[0]);
						...
						$scope.changeCarouselSlides = function() {
							...
							for (var i = 0; i < Object
									.keys($scope.carouselConfig).length; i++) {
								if ($scope.carouselConfig[i].slideEnabled == 'true') {
									singleSlideFlag = false;
									break;
								} else if (i == Object
										.keys($scope.carouselConfig).length - 1) {
						...
						$scope.validateImageSize = function(file, height, width) {
							if (file.height > height || file.width > width) {
								...
								$scope.msg = "Image size should be less than "
						...
						$scope.saveCarouselPicture = function(key, picture) {
							...
							if (validFormats.indexOf(ext) == -1) {
								...
								var reader = new FileReader();
								reader.onload = function(event) {
									var data = reader.result.split('base64,')
											.pop();
						...
						$scope.addEventSlide = function() {
							...
							if ((!$scope.eventCarousel.backGround || !$scope
									.validateImageSize(
											$scope.eventCarousel.backGround,
											2560, 524))
									&& (!$scope.eventCarousel.infoGraphic || !$scope
											.validateImageSize(
								...
								slide['bgImgKey'] = "carousel.event."
										+ slide['uniqueKey'] + ".bgImg";
								slide['infoImgKey'] = "carousel.event."
										+ slide['uniqueKey'] + ".infoImg";
								slide['hasInfoGraphic'] = $scope.eventCarousel.hasInfoGraphic
										|| ($scope.eventCarousel.infoGraphic != null);
								...
								if (typeof $scope.eventConfig === "undefined") {
									...
									apiService
											.updateSiteConfig("event_carousel",
													reqObj)
											.then(
													function(response) {
														$scope.msg = "Carousel Updated successfully.";
						...
						$scope.editEventSlide = function(key, val) {
							...
							if ($scope.eventSC == "") {
								...
								$scope.eventSC = val['supportingContent']
										.replace(/<(?:.|\n)*?>/gm, '');
								$scope.keyval = key;
								$scope.showEventSlidesPopup();
							}
						...
						$scope.deleteEventSlide = function() {
							...
							var key = $scope.deleteKey;
							$scope
									.deleteCarouselPicture($scope.eventConfig[key]['bgImgKey']);
							$scope
									.deleteCarouselPicture($scope.eventConfig[key]['infoImgKey']);
							delete $scope.eventConfig[key];

							var carouselConfigStr = JSON
									.stringify($scope.eventConfig);
							var convertedString = carouselConfigStr.replace(
									/"/g, '\"');
							...
							apiService.updateSiteConfig("event_carousel",
									reqObj).then(function(response) {
								$scope.closePoup();
								$scope.getEventCarousel();
								$scope.msg = "Carousel Updated successfully.";
						...
						$scope.changeEventSlideOrder = function(ev) {
							...
							if (isNaN($scope.order.changeOrderValue)
									|| $scope.order.changeOrderValue > Object
											.keys($scope.eventConfig).length - 1
									|| $scope.order.changeOrderValue < 0) {
						...
						$scope.addStorySlide = function() {
							...
							slide['supportingContent'] = slide_supportingContent
									.trim();
							// slide['infoImageAling']=
							// $scope.carousel_Info_Aling;
							// slide['textAling']= $scope.carousel_Text_Aling;

							// slide['graphicImgEnabled'] =
							// $scope.successCarousel.graphicImg;
							slide['slideEnabled'] = "true";
							...
							// if carouselConfig is not present in DB create a
							// record else update the record
							...
							if (typeof $scope.storyConfig === "undefined") {
								...
								apiService
										.updateSiteConfig("story_carousel",
												reqObj)
										.then(
												function(response) {
													$scope.msg = "Carousel Updated successfully.";
						...
						$scope.changeStorySlideOrder = function(ev) {
							...
							if (isNaN($scope.order.changeOrderValue)
									|| $scope.order.changeOrderValue > Object
											.keys($scope.storyConfig).length - 1
									|| $scope.order.changeOrderValue < 0) {
							...
							apiService.updateSiteConfig("story_carousel",
									reqObj).then(function(response) {
								$scope.getEventCarousel();
								$scope.msg = "Carousel Updated successfully.";
								$scope.icon = '';
						...
						$scope.editStorySlide = function(key, val) {
							...
							$scope.successSC = val['supportingContent']
									.replace(/<(?:.|\n)*?>/gm, '');
						...
						$scope.getSearch = function() {
							...
							apiService
									.getRightToUse(rtuReferenceId, solName)
									.then(
											function successCallback(response) {
