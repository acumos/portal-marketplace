...
public class GatewayClient extends AbstractClient {
	...
	public JsonResponse<MLPPeer> ping(String peerId) throws HttpStatusCodeException {
		...
		try {
			...
			log.error(EELFLoggerDelegate.errorLogger, uri + " failed" + ((response == null) ? "" : (" " + response)),
					x);
			String resp = x.getResponseBodyAsString();
			String message = resp.substring(resp.indexOf("\"error\":\"") + 9, resp.indexOf("\",\"exception\""));
			throw new HttpClientErrorException(x.getStatusCode(), message);
		} catch (Throwable t) {
