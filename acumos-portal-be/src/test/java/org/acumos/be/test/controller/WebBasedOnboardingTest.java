...
public class WebBasedOnboardingTest {
	...
	public void testAddToCatalog() {
		...
		try {
			...
			MLPUser requestUser = new MLPUser();
			requestUser.setUserId(userId);
			requestUser.setApiToken("xyz");
			requestUser.setAuthToken("a2z");
			request.setAttribute("mlpuser", requestUser);
			value = webBasedController.addToCatalog(request, response, null, null, restPageReq, userId);
			logger.equals(value);
